...
type TileProps = {
  ...
  disabled?: boolean;
  onClick?: () => void;
  /** Makes the tile a file-input wrapper */
  fileInput?: React.ReactNode;
  accent?: "slate" | "amber" | "violet" | "emerald";
  dashed?: boolean;
};
...
function ActionTile({
  ...
  const hoverCls = ACCENT_HOVER[accent];

  const inner = (
    <div
      className={`flex flex-col items-center justify-center gap-3 rounded-2xl border bg-white px-3 transition-all active:scale-[0.97] select-none
        ${dashed ? "border-dashed" : ""}
        ${loading ? "opacity-70 cursor-wait" : disabled ? "opacity-40 cursor-not-allowed" : `cursor-pointer ${hoverCls}`}
        border-slate-200`}
      style={{ minHeight: 112 }}
    >
      <span className={`flex items-center justify-center w-11 h-11 rounded-xl ${iconBg} transition-colors`}>
...
export default function DealIntakeActions({ dealId, isDriveConnected }: Props) {
  ...
  if (!isDriveConnected) {
    return (
      <div className="rounded-2xl border border-slate-100 bg-white shadow-sm px-5 py-4 mb-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
              Required to capture photos, record audio, and upload files.
            </p>
            <Link
              href="/settings/integrations"
              className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition-colors"
              style={{ minHeight: 44 }}
            >
              Connect Drive
            </Link>
  ...
  return (
    <div
      className={`rounded-2xl border bg-white shadow-sm px-4 py-4 mb-4 transition-all ${
        isDragOver ? "border-indigo-300 bg-indigo-50/20 ring-2 ring-indigo-100" : "border-slate-100"
      }`}
      onDrop={handleDrop}
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <ActionTile
          label="Paste Text"
          sublabel="Notes, emails, listings"
          accent="slate"
          icon={
        ...
        <ActionTile
          label="Upload File"
          sublabel="PDF, doc, spreadsheet"
          accent="emerald"
          loading={uploading === "file"}
          disabled={!!uploading && uploading !== "file"}
          dashed
          icon={
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
